...
export default function AdminEdit({ post, onUpdate }: DetailCardPostProps) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        const cvPostId = post.post_id

        formData.append('post_by', cvPostId.toString());
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/post/${post.post_id}`, {
            method: 'PUT',
            body: formData,
        });
        ...
        if (res.ok) {
            ...
            if (onUpdate) onUpdate(); // ✅ เรียก callback เพื่ออัปเดต dashboard
        }
                            ...
                            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                                ...
                                <ModalBody>
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <Input label="หัวเรื่อง" type="text" size="sm" defaultValue={post.post_header} onChange={(e) => setPostHeader(e.target.value)} />
                                        <Textarea className="w-full" label="เนื้อหา" defaultValue={post.post_des} onChange={(e) => setPostDes(e.target.value)} />
